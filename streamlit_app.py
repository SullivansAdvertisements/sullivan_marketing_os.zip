...
with tabs[2]:
    st.header("Working Google Trends Research Center — Max Results + Fallback")
        ...
        with st.spinner("Building clean trend tables..."):
            try:
                if mode == "Demo Mode":
                    ...
                    status = st.session_state["research"].get("status", "unknown")
                    msg = st.session_state["research"].get("message", "Research loaded.")
    ...
    if research:
        status_msg = research.get("message", "")
        status_type = research.get("status", "")
        if status_type == "live_pytrends":
            st.success(status_msg)
        elif status_type == "fallback":
            st.warning(status_msg)
            if research.get("pytrends_error"):
                with st.expander("Why pytrends did not load"):
